...
Created to design experiments for GRU model for Univariate analysis of one single marker from MoCap data.
Using command line arguments different experiments can be ran with different size of timestep and batches size.
This uses all datasets for lifting which are 8 for training and 2 for testing.
"""
import os
...
experiment_set = f'GRU-1L-SGD-MSE-Batch{BATCH_SIZE}_Experiments'
...
new_dir = f'GRU_Model_T{TIMESTEPS}_B{BATCH_SIZE}'
...
def fit_stateless_gru(model, batch_size, num_epoch=10, valid_data=None, v_split=None, valid_train=True,
    ...
    """
    for epoch_i in range(num_epoch): # each epoch is looped individually to allow for each file to be trained on
        for file_index, file in enumerate(training_file_list):
            ...
            if True is valid_train:
                ...
                model.fit(trainX, trainY, epochs=num_epoch, batch_size=batch_size, verbose=0, shuffle=False)
...
#              'epochs', 'batch size', 'train mse', 'train rmse', 'train mae',
...
def run_model(experiment_counter=0):
    ...
    png_name = f'GRU_Model_T{TIMESTEPS}_B{BATCH_SIZE}_Experiment_{experiment_counter}_{marker_name}'
    ...
    fit_stateless_gru(gru_model, batch_size=batch_size, num_epoch=num_epochs, v_split=v_split, valid_train=True,
    ...
    for file_index in range(len(testing_ds_dict)):
        ...
        png_name = f'GRU_Model_T{TIMESTEPS}_B{batch_size}_Experiment_{experiment_counter}_File_{filename_id}_{marker_name}'
        mocap_data, testX_scaled, testY, scaler= fetch_test_data(file_index)

        y_pred = gru_model.predict(testX_scaled, batch_size=1)
        ...
        excel_name = f"GRU_T{TIMESTEPS}_B{batch_size}_Experiments_{marker_name}_Results.xlsx"
