...
hyperparameter searching for GRU model to find for each marker what type parameters might work best.
...
experiment_set = 'GRU-1L-MSE-HYPERExperiments'
...
new_dir = f'GRU_Stateless_Model_T{TIMESTEPS}'
...
def fit_gru(model,trainX, trainY, batch_size, num_epoch=10, valid_data=None, v_split=None, valid_train=True,
...
def run_model(experiment_counter=START):
    ...
    for config in configs[START:END]:
        ...
        # use the gru model and define it which would be different for each depending on number of neurons
        print("define gru")
        ...
        fit_gru(gru_model, trainX=trainX, trainY=trainY, batch_size=batch_size, num_epoch=num_epochs,
                           v_split=v_split,
                           valid_train=True)
        ...
        plot_metrics(file_path_to_save_results, png_name, repeat_each_epoch=False,
                     model_metric_history=gru_model.history.history,
                     valid_train=True)
        ...
        for i in range(len(testX)):
            ...
            y_pred = one_step_forcast(gru_model, testX[i], batch_size)
            # invert scaling
        ...
        plot_single_mocap_file_predictions(file_path=file_path_to_save_results, png_name=png_name, raw_dataset=mocap_data,
                                           train_predict=train_predict_unscaled, test_predict=test_predictions, look_back=TIMESTEPS, increments=INCREMENTS)
        ...
        """gru_stateless_dict rows_keys: ['Name', 'Marker', 'loss function', 'optimizer', 'traintest split percent', 'validation split', 'hidden_units',
                 'timesteps','epochs', 'batch size', 'train mse', 'train rmse', 'train mae'
                 'test mse', 'test rmse', 'test mae']"""
        gru_stateless_dict['Name'].append(png_name)
        gru_stateless_dict['Marker'].append(marker_name)
        gru_stateless_dict['loss function'].append(loss_function)
        gru_stateless_dict['optimizer'].append(opt)
        gru_stateless_dict['traintest split percent'].append(split_percent)
        gru_stateless_dict['validation split'].append(v_split)
        gru_stateless_dict['hidden_units'].append(num_neurons)
        gru_stateless_dict['timesteps'].append(TIMESTEPS)
        gru_stateless_dict['epochs'].append(num_epochs)
        gru_stateless_dict['batch size'].append(batch_size)
        gru_stateless_dict['train mse'].append(train_mse.numpy())
        gru_stateless_dict['train rmse'].append(train_rmse)
        gru_stateless_dict['train mae'].append(train_mae.numpy())
        gru_stateless_dict['test mse'].append(test_mse.numpy())
        gru_stateless_dict['test rmse'].append(test_rmse)
        gru_stateless_dict['test mae'].append(test_mae.numpy())

        """Save the results to exxcel file begins"""
        # new dataframes for the columns in each sheet
        Excel_df = pd.DataFrame(gru_stateless_dict)
        # new dictionary for all the sheets int he excel file
        excel_sheet_name = f'GRU_Stateless_{subject_id}'
        excel_sheet_3 = {excel_sheet_name: Excel_df}
        gru_stateless_dict.clear()
        # name of excel file with summary
        excel_name = f"{subject_id}_StartAt_{START}_GRU_Stateless_T{TIMESTEPS}_Experiments_{marker_name}_Results.xlsx"
...
if __name__ == "__main__":
    ...
    faulthandler.enable()
    experiment_counter = START  # 3 * 20 * 2 * 2 * 7 = 1680 files so thats the last counter value
