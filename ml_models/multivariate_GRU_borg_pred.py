...
This implementation uses the GRU Deep learning model.
"""
import os
...
parser.add_argument('num_layers', type=int, help='an integer for value of how many layers for gru model (max5)')
...
from ml_models.models import define_gru
...
experiment_set = f'GRU-{NUM_LAYERS}L-Batch{BATCH_SIZE}_BorgPrediction_Experiments'
...
new_dir = f'GRU_Model_T{TIMESTEPS}_B{BATCH_SIZE}_{NUM_LAYERS}L'
...
def fit_gru(model, batch_size, num_epoch=10, valid_data=None, v_split=None, valid_train=True,
    ...
    for epoch_i in range(num_epoch):
        ...
        for file_index, file_in_ds in enumerate(training_file_names): # iterate through each file
            """The GRU network expects the input data (X) to be provided 
...
def run_model(experiment_counter=0):
    ...
    png_name = f'GRU_Model_T{TIMESTEPS}_B{BATCH_SIZE}_{NUM_LAYERS}L_Borg_Prediction_Experiment_{experiment_counter}_{marker_name}'
    # defining the model that will be used
    gru_model = define_gru(timesteps=TIMESTEPS, num_features=NUM_FEATURES, num_units=num_neurons,
                             stateful=STATEFUL,num_layers=NUM_LAYERS)

    # flag for plotting
    repeat_each_epoch = True
    # training using model created
    fit_gru(gru_model, batch_size=batch_size, num_epoch=num_epochs, valid_train=False,
    ...
    with open(summary_model_path, 'w+') as fh:
        ...
        gru_model.summary(print_fn=lambda x: fh.write(x + '\n'))
    ...
    for training_file_index in range(len(training_ds_dict)):
        ...
        train_png_name = f'GRU_Model_T{TIMESTEPS}_B{BATCH_SIZE}_{NUM_LAYERS}L_Borg_Prediction_Experiment_{experiment_counter}_File_{training_filename_id}_{marker_name}'
        ...
        y_borg_pred = gru_model.predict(trainX_scaled, batch_size=1)
        ...
        GRU_dict['Name'].append(train_png_name)
        GRU_dict['Marker'].append(marker_name)
        GRU_dict['loss function'].append(loss_function)
        GRU_dict['optimizer'].append(opt)
        GRU_dict['validation split'].append(v_split * 100)
        GRU_dict['hidden_units'].append(num_neurons)
        GRU_dict['timesteps'].append(TIMESTEPS)
        GRU_dict['epochs'].append(num_epochs)
        GRU_dict['batch size'].append(batch_size)
        GRU_dict['test mse'].append(train_mse.numpy())
        GRU_dict['test rmse'].append(train_rmse)
        GRU_dict['test mae'].append(train_mae.numpy())

        # new dataframes for the columns in each sheet
        GRU_df = pd.DataFrame(GRU_dict)
        # new dictionary for all the sheets int he excel file
        train_excel_sheet_name = f'GRU_{training_filename_id}'
        train_excel_sheet_3 = {train_excel_sheet_name: GRU_df}
        GRU_dict.clear()
        # name of excel file with summary
        train_pred_excel_name = f"GRU_T{TIMESTEPS}_B{BATCH_SIZE}_{NUM_LAYERS}L_Train_Borg_Prediction_Experiments_{marker_name}_Results.xlsx"
    ...
    for file_index in range(len(testing_ds_dict)):
        ...
        # name that will be used for storing file names corresponding to the experiment ran
        test_png_name = f'GRU_Model_T{TIMESTEPS}_B{BATCH_SIZE}_{NUM_LAYERS}L_Borg_Prediction_Experiment_{experiment_counter}_File_{filename_id}_{marker_name}'
        ...
        y_borg_pred = gru_model.predict(testX_scaled, batch_size=1)
        ...
        GRU_dict['epochs'].append(num_epochs)
        GRU_dict['batch size'].append(batch_size)
        GRU_dict['test mse'].append(test_mse.numpy())
        GRU_dict['test rmse'].append(test_rmse)
        GRU_dict['test mae'].append(test_mae.numpy())

        # new dataframes for the columns in each sheet
        GRU_df = pd.DataFrame(GRU_dict)
        # new dictionary for all the sheets int he excel file
        excel_sheet_name = f'GRU_{filename_id}'
        excel_sheet_3 = {excel_sheet_name: GRU_df}
        GRU_dict.clear()
        # name of excel file with summary
        excel_name = f"GRU_T{TIMESTEPS}_B{BATCH_SIZE}_{NUM_LAYERS}L_Test_Borg_Prediction_Experiments_{marker_name}_Results.xlsx"
